...
```mermaid
flowchart LR
    MAT["Materiality<br/>risks scored"] -- "risk register" --> DEC["Disclosure<br/>d* · Λ"]
...
HongERP는 ESG 플랫폼이 비워 둔 **의사결정 층**을 채우는 프로토타입입니다. 기존 도구는 회사가 무엇을 공시했는지 기록하는 데서 멈춥니다. 이 시스템은 회사의 위험 노출을 입력받아 **얼마나 헤지하고 얼마나 공시할지**를 함께 계산합니다.

구조는 두 겹입니다. 아래층에는 연구 논문 네 편의 결과를 그대로 옮겨 고정한 계산 엔진이 있습니다. 예산 배분, 배리어 옵션 가격 산정, 현금흐름위험회피 원장, 공시와 헤지의 균형 모형이 여기에 속합니다. 위층에는 실제 회사의 결재선을 본뜬 통제 구조가 있습니다. 사업부장이 지표를 **상신**하면 감사팀이 **승인**하거나 **반려**하고, 자금부가 파생상품을 **체결**하며, CFO가 회계 **지정**과 **기말 마감**을 맡습니다. 모든 처리는 지울 수 없는 감사 기록으로 남고, 수치를 올린 사람은 그 수치를 승인할 수 없습니다.

화면 아홉 개는 하나의 포지션을 공유합니다. 중대성 평가가 확정한 위험 목록을 배경으로 공시 문제가 위험의 가격 Λ를 정하고, 예산과 데스크는 그 가격에 맞춰 움직입니다. 장부의 지정 결과는 다시 공시 연구의 결과변수로 돌아옵니다. 핵심 수치는 전부 독립 검증을 거쳤습니다. 균형 엔진은 별도 최적화기와 200회 대조해 오차가 3×10⁻⁶ 이하이고, 옵션 가격은 풋-콜 패리티를 10⁻¹⁴ 수준으로 만족하며, 백테스트는 실제 유가와 환율 40년치로 매달 다시 계산합니다.

## Run it locally

```bash
git clone https://github.com/JihongParker/hong-erp.git
cd hong-erp
npm install
npm run dev      # then open http://localhost:5173 in your browser
npm run build    # production build to dist/
```

The dev server keeps running in that terminal; stop it with Ctrl+C. If you just want to see the app, the [live demo](https://jihongparker.github.io/hong-erp/) is always current.
...
Papers: **[SSRN author page](https://ssrn.com/author=12066228)** · code and TeX: **[wti-fx-hedge-program](https://github.com/JihongParker/wti-fx-hedge-program)**
