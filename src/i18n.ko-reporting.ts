...
export const KO_REPORTING: Record<string, string> = {
  ...
  'Variance removed — walk-forward, out of sample': '지워 낸 변동성 — 워크포워드 · 표본 밖 검증',
  'vs naive': '단순 분할 대비',
  'gap to oracle': '오라클 격차',
  'Policy': '정책',
  'Ann. vol': '연율 변동성',
  'Variance removed': '지워 낸 변동성',
  'Max drawdown': '최대 낙폭(MDD)',
  'Cost': '비용',
  'Unhedged': '무헤지',
  'Naive even split': '단순 균등 분할',
  'Full-sample MV (oracle)': '전체 표본 MV (오라클)',
  'Walk-forward MV': '워크포워드 MV',
  'carry the whole two-factor exposure': '유가·환 위험을 전부 그대로 안고 감',
  'split the budget evenly across both legs': '예산을 두 레그에 균등 분할',
  'look-ahead: MV split on the full sample (upper bound)': '미래를 미리 본 배분 — 도달 가능한 상한선',
  'covariance-aware split, estimated on strictly past data': '공분산 반영 분할 · 과거 데이터만으로 추정',
  ...
  'P2 — the dynamic hedge-cost parabola, and why naive c = 1 is off-optimum':
    'P2 — 동적 헤지 비용 포물선, 단순한 c = 1이 최적이 아닌 이유',
