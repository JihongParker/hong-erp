...
export const KO_REPORTING: Record<string, string> = {
  ...
  'Demo taxonomy. Mappings are not validated for practice. The point is the structure: hierarchical codes mapped to multiple frameworks at the datapoint level.':
    '데모 분류체계입니다. 매핑은 실무 검증을 거치지 않았습니다. 핵심은 구조입니다: 계층적 코드를 데이터포인트 수준에서 여러 프레임워크에 매핑합니다.',
  'Select an account on the left': '왼쪽에서 계정을 선택하세요',
  ...
  'Operational disruption in water-stressed regions': '물 부족 지역의 운영 중단',
  ...
  // ── Metrics entry ──
  'New submission': '신규 제출',
  'Year': '연도',
  'Value': '값',
  'Evidence attached (mockup)': '증빙 첨부 (목업)',
  '✕ Value must be a non-negative number': '✕ 값은 0 이상의 숫자여야 합니다',
  'Submit for review': '검토 요청',
  'Import CSV': 'CSV 가져오기',
  'Download template': '템플릿 다운로드',
  'Approval queue': '승인 대기열',
  'Queue is clear.': '대기열이 비어 있습니다.',
  'Approve': '승인',
  'Reject': '반려',
  'Recent activity': '최근 활동',
  'Export CSV': 'CSV 내보내기',
  'Switch to the Division head role to import': '가져오려면 사업부장 역할로 전환하세요',
  'Bulk submit from a CSV (division,datapoint,year,value) — simple CSV, no quoted fields':
    'CSV(division,datapoint,year,value)로 일괄 제출 — 따옴표 없는 단순 CSV',
  'Download every approved metric (all divisions) as CSV': '승인된 모든 지표(전 사업부)를 CSV로 다운로드',
  'FY closed — adjustments must be new events': '마감된 회계연도 — 조정은 새 이벤트로 처리해야 합니다',
  'Switch to the Audit role to approve': '승인하려면 감사 역할로 전환하세요',
  'Switch to the Audit role to reject': '반려하려면 감사 역할로 전환하세요',
  // status labels
  'Pending review': '승인 대기',
  'Approved': '승인됨',
  'Rejected': '반려됨',
  ...
  "These are the divisions' live parameter records: edits persist and flow to the division book on the Decision Dashboard.":
    '사업부별 실시간 파라미터 레코드입니다: 수정 사항은 저장되어 의사결정 대시보드의 사업부 장부로 흘러갑니다.',
  "How to read it: divisions with larger exposure (σf) and faster penalty attenuation (k) optimally disclose more, and because disclosure lowers the shadow price Λ of residual risk, their hedge ratios move with it — disclosure and hedging are two solutions of one problem. Try raising a division's mandated floor d̲ past its voluntary d* and watch the hedge fall: that is the crowding-out regime.":
    '읽는 법: 익스포저(σf)가 크고 페널티 감쇠(k)가 빠른 사업부일수록 최적 공시량이 많아집니다. 그리고 공시는 잔여 리스크의 잠재가격 Λ를 낮추므로 헤지 비율도 그에 따라 함께 움직입니다 — 공시와 헤지는 하나의 문제에 대한 두 해입니다. 어느 사업부의 의무 하한 d̲를 자율 d*보다 높게 올려 보세요. 헤지가 내려갑니다: 이것이 구축(crowding-out) 국면입니다.',
  ...
  'No events match.': '조건에 맞는 이벤트가 없습니다.',
  ...
  'Variance removed — walk-forward, out of sample': '제거된 분산 — 워크포워드, 표본 밖',
  'vs naive': '나이브 대비',
  'gap to oracle': '오라클 격차',
  'Policy': '정책',
  'Ann. vol': '연율 변동성',
  'Variance removed': '제거된 분산',
  'Max drawdown': '최대 낙폭',
  'Cost': '비용',
  'Unhedged': '무헤지',
  ...
  'carry the whole two-factor exposure': '2-요인 익스포저 전체를 그대로 부담',
  'split the budget evenly across both legs': '예산을 두 다리에 균등 분할',
  'look-ahead: MV split on the full sample (upper bound)': '미래 참조: 전체 표본에 대한 MV 분할 (상한)',
  ...
  'P2 — the dynamic hedge-cost parabola, and why naive c = 1 is off-optimum':
    'P2 — 동적 헤지 비용 포물선, 그리고 나이브 c = 1이 최적이 아닌 이유',
  'Hedge-cost standard deviation vs coupling c': '커플링 c에 대한 헤지 비용 표준편차',
  'Rolling correlation and FX coverage': '롤링 상관계수와 FX 커버리지',

  // ── Command palette ──
  'Type a command — jump to a module, switch role, start the tour…':
    '명령을 입력하세요 — 모듈 이동, 역할 전환, 투어 시작…',
  'No matches': '일치 항목 없음',
